"""Playwright real Guadalajara — predial + multas Jalisco estatal.

REFACTORIZADO 2026-06-13: predial consulta catálogo central que apunta a
`pagoenlinea.guadalajara.gob.mx/impuestopredial/` (Angular Material, validado MCP).
"""
...
def predial_real(cuenta_predial: str) -> dict[str, Any]:
    """Consulta predial Guadalajara via catálogo central (Angular Material validado)."""
    cfg = get_municipio_config("jal", "guadalajara")
    if cfg is None or not cfg.portal_predial_url:
        raise UpstreamError(
            "Guadalajara no tiene URL de predial en catálogo.",
            {"municipio": "guadalajara"},
        )
    portal_cfg = cfg.to_predial_config()
    if portal_cfg is None:
        raise UpstreamError("Guadalajara: PortalConfig falló.", {})
    return consulta_portal(portal_cfg, cuenta_predial)


def multas_real(placa: str) -> dict[str, Any]:
    """Consulta multas Jalisco (estatal)."""
